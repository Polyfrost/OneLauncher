        ...
        <div className="h-full">
          ...
          >
            <div className="grid grid-cols-[repeat(auto-fill,minmax(350px,1fr))] w-full transform-gpu gap-2">
              {arr.map((data) => (
                <ScreenshotEntry key={data} path={`screenshot-${data}.png`} cluster_path="cluster" />
              ))}
            </div>
          </Show>
        </div>
...
function ScreenshotEntry(props: { path: string; cluster_path: string }) {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <img alt={props.path} className="aspect-video w-full rounded-lg" src="https://github.com/emirsassan.png" />
    </div>
  );
}
