...
function RouteComponent() {
  ...
  if (usersLoading || defaultUserLoading) {
    return <Sidebar.Page><h1>Accounts</h1><p>Loading accounts...</p></Sidebar.Page>;
  }
    ...
    <Sidebar.Page>
      ...
      <ScrollableContainer>
        <div className="h-full space-y-2">
          <Show when={usersData && usersData.length > 0} fallback={<p>No accounts found.</p>}>
...
function AccountRow({ account, isCurrent }: AccountRowProps) {
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleSetDefault(); }}
    >
