...
use crate::utils::{http::{fetch_advanced, fetch_json}, io::{self, IOError}, java::{self, get_java_version, JavaVersion}};
use crate::State;

pub async fn filter_java_version(
...
pub async fn install_java(java_version: u32) -> crate::Result<PathBuf> {
    ...
    let ingress = init_ingress(
        crate::IngressType::DownloadJava { version: java_version },
        100.0, "downloading java version").await?;
    ...
    if let Some(download) = packages.first() {
        ...
        archive.extract(&path).map_err(|err| IOError::from_zip(err))?;
        send_ingress(&ingress, 10.0, Some("extracted java binary")).await?;
        let mut base_path = path.join(download.name.file_stem().unwrap_or_default().to_string_lossy().to_string());
...
pub async fn get_max_memory() -> crate::Result<u64> {
    Ok(sys_info::mem_info()
        .map_err(|_| anyhow::anyhow!("failed to get computer memory"))?.total)
}
