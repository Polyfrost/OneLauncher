use super::IngressId;
use crate::api::proxy::{ProxyError, Ingress, IngressType, ProcessPayloadType, ClusterPayloadType, InternetPayload};
use crate::store::{ClusterPath, IngressProcessType, IngressProcessor};
use uuid::Uuid;

#[cfg(feature = "tauri")]
use crate::api::proxy::{IngressPayload, ProcessPayload, ClusterPayload, MessagePayload};
...
pub async fn init_ingress_internal(
    ...
    proxy_state.ingress_feeds.write().await.insert(key.0, Ingress {
        ingress_uuid: key.0,
...
pub async fn send_ingress(
    ...
    let display_conv = if display >= 1.0 {
        None
    } else {
        Some(display)
    };
    ...
    if f64::abs(display - ingress.last_sent) > 0.005 {
        #[cfg(feature = "cli")]
        {
            ingress.cli.set_message(message.map(|x| x.to_string()).unwrap_or(ingress.message.clone()));
            ingress.cli.set_position((display * CLI_TOTAL_INGRESS as f64).round() as u64);
        }

        #[cfg(feature = "tauri")]
        proxy_state.app.emit("ingress", IngressPayload {
...
pub async fn send_message(message: &str) -> crate::Result<()> {
    ...
    {
        let proxy_state = crate::ProxyState::get().await?;
        proxy_state.app.emit("message", MessagePayload { message: message.to_string() }).map_err(ProxyError::from)?;
    }
...
pub async fn send_offline(offline: bool) -> crate::Result<()> {
    ...
    {
        let proxy_state = crate::ProxyState::get().await?;
        proxy_state.app.emit("offline", offline).map_err(ProxyError::from)?;
    }
...
pub async fn send_internet(internet: InternetPayload) -> crate::Result<()> {
    ...
    {
        let proxy_state = crate::ProxyState::get().await?;
        proxy_state.app.emit("internet", internet).map_err(ProxyError::from)?;
    }
...
pub async fn send_process(
    ...
    {
        let proxy_state = crate::ProxyState::get().await?;
        proxy_state.app.emit("process", ProcessPayload { uuid, pid, event, message: message.to_string() }).map_err(ProxyError::from)?;
    }
...
pub async fn send_cluster(
    ...
    #[cfg(feature = "tauri")]
    {
        ...
        let proxy_state = crate::ProxyState::get().await?;
        proxy_state.app.emit("cluster", ClusterPayload {
            uuid,
