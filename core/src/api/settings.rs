...
pub async fn set_directory(new: PathBuf) -> crate::Result<()> {
    ...
    if !new.is_dir() {
        return Err(anyhow::anyhow!("new config directory {} is not a folder!", new.display()).into());
    }

    if !is_writable(new.clone()).await? {
        return Err(anyhow::anyhow!("new config directory {} is not writeable", new.display()).into());
    }

    let ingress = init_ingress(
        crate::IngressType::SyncConfig { new_path: new.clone() },
        100.0, "syncing and changing configuration directory",
    ).await?;
    ...
    while let Some(e) = cfg_entries.next_entry().await.map_err(|err| crate::utils::io::IOError::with_path(err, &old))? {
        let epath = e.path();
        if let Some(file_name) = epath.file_name() {
            if file_name == store::CLUSTERS_FOLDER
                || file_name == store::METADATA_FOLDER
            {
    ...
    for epath in entries {
        let relpath = epath.strip_prefix(&old).map_err(|err| anyhow::anyhow!("failed to strip prefix {err}"))?;
    ...
    if cleanable_nentries > 0 { tracing::trace!("deleting old config files"); }
    for ce in cleanable {
    ...
    tracing::info!("successfully switched configuration directory to {}", new.display());
