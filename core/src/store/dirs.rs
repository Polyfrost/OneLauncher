...
impl Directories {
    ...
    pub fn init_settings_dir() -> Option<PathBuf> {
        Self::env_path("ONELAUNCHER_CONFIG").or_else(|| Some(dirs::config_dir()?.join(constants::APP_CONFIG_DIR)))
    }
    ...
    pub fn initalize(settings: &Settings) -> crate::Result<Self> {
        ...
        let settings_dir = Self::init_settings_dir().ok_or(DirectoryError::ConfigDir)?;
        let config_dir = settings.config_dir.clone().ok_or(DirectoryError::ConfigDir)?;
        Ok(Self {
            settings_dir,
