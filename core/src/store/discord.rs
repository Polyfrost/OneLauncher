...
impl DiscordRPC {
    ...
    pub async fn initialize(is_offline: bool) -> crate::Result<Self> {
        let mut discord_ipc =
            DiscordIpcClient::new(DISCORD_RPC_CLIENT_ID).map_err(|e| {
    ...
    pub async fn set_activity(&self, msg: &str, reconnect_if_fail: bool) -> crate::Result<()> {
        if !self.retry_online().await { return Ok(()); }
    ...
    pub async fn apply_activity(&self, msg: &str, reconnect_if_fail: bool) -> crate::Result<()> {
        if !self.retry_connection().await { return Ok(()); }

        let activity = Activity::new().state(msg).assets(
            Assets::new().large_image("polyfrost").large_text("Polyfrost Logo")
        );

        let mut client: tokio::sync::RwLockWriteGuard<'_, DiscordIpcClient> = self.client.write().await;
        ...
        if reconnect_if_fail {
            if let Err(_e) = result {
                client.reconnect().map_err(|e| {
                    anyhow::anyhow!("failed to reconnect to Discord RPC {}", e)
                })?;
    ...
    pub async fn clear_activity(&self, reconnect_if_fail: bool) -> crate::Result<()> {
        if !self.retry_online().await || !self.retry_connection().await { return Ok(()); }
