...
use crate::proxy::send::init_ingress_internal;
use crate::proxy::send::send_ingress;
use crate::proxy::send::send_offline;
use crate::utils::http;
use crate::utils::http::FetchSemaphore;
...
impl State {
    ...
    pub async fn get() -> crate::Result<Arc<tokio::sync::RwLockReadGuard<'static, Self>>> {
        Ok(Arc::new(LAUNCHER_STATE.get_or_try_init(Self::initialize).await?.read().await))
    }
    ...
    pub async fn get_and_write() -> crate::Result<tokio::sync::RwLockWriteGuard<'static, Self>> {
        Ok(LAUNCHER_STATE.get_or_try_init(Self::initialize).await?.write().await)
    }
    ...
    async fn initialize() -> crate::Result<RwLock<State>> {
        let ingress = init_ingress_internal(
            crate::IngressType::Initialize,
            100.0,
            "initializing state",
        ).await?;
        ...
        let fetch_semaphore = FetchSemaphore(RwLock::new(Semaphore::new(settings.max_async_fetches)));
        let io_semaphore = IoSemaphore(RwLock::new(Semaphore::new(settings.max_async_io_operations)));
        send_ingress(&ingress, 10.0, None).await?;
