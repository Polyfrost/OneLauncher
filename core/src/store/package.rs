...
use crate::{prelude::IOError, utils::http::{write_icon, FetchSemaphore, IoSemaphore}};
...
pub async fn generate_context(
    ...
    for path in paths {
        ...
        if let Some(ext) = path.extension() { if ext == "txt" { continue; } }
        let mut file = tokio::fs::File::open(path.clone()).await.map_err(|e| IOError::with_path(e, &path))?;
...
impl PackageType {
    ...
    pub fn from_loader(loaders: Vec<String>) -> Option<Self> {
        if loaders.iter().any(|x| ["fabric", "forge", "quilt"].contains(&&**x)) {
        ...
        } else if loaders.iter().any(|x| ["vanilla", "canvas", "minecraft"].contains(&&**x)) {
...
async fn read_icon(
    ...
    if let Some(icon_path) = icon_path {
        ...
        if let Ok(zip_reader) = zip_reader {
            let zip_idx_res = zip_reader.file().entries().iter().position(|f| {
                f.filename().as_str().unwrap_or_default() == icon_path
            });

            if let Some(zip_idx) = zip_idx_res {
                let mut bytes = Vec::new();
                if zip_reader.reader_with_entry(zip_idx).await?.read_to_end_checked(&mut bytes).await.is_ok() {
