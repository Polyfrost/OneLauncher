...
impl ChildType {
    ...
    pub async fn cache(&self, uuid: Uuid, cluster_path: ClusterPath, post: Option<String>) -> crate::Result<()> {
        let pid = match self {
        ...
        let process = system.process(sysinfo::Pid::from_u32(pid)).ok_or_else(|| {
            anyhow::anyhow!("could not find process {}", pid)
        })?;
        ...
        let mut caches = if let Ok(proc_json) =
            read_json::<HashMap<Uuid, ProcessorCache>>(
                &path,
                &state.io_semaphore,
            ).await
        {
...
impl Processor {
    ...
    pub async fn running(&self) -> crate::Result<Vec<Uuid>> {
        ...
        for key in self.keys() {
            if let Some(process) = self.get(&key) {
                ...
                if process.current_child.write().await.try_wait().await?.is_none() {
                    keys.push(key);
                }
            }
        }
    ...
    pub async fn restore(&mut self) -> crate::Result<()> {
        ...
        let mut processor_caches = if let Ok(processes_json) =
            read_json::<HashMap<uuid::Uuid, ProcessorCache>>(
                &processor_path,
                &state.io_semaphore,
            )
            .await
        {
            ...
            crate::utils::http::write(&processor_path, &serde_json::to_vec(&zeros)?, &state.io_semaphore).await?;
            processes_json
        } else {
            HashMap::new()
        };
    ...
    pub async fn insert_process(
        ...
        let pid = process.id().ok_or_else(|| anyhow::anyhow!("process failed, couldn't get pid"))?;
        process.cache(uuid, cluster_path.clone(), post.clone()).await?;
        ...
        let child = ProcessorChild { uuid, cluster_path, current_child, manager, last_updated };
    ...
    pub async fn insert_restored(
        ...
        // ensure located stray process matches with our pid recorded process
        {
            ...
            if cache.start_time != process.start_time() {
                return Err(anyhow::anyhow!("restored process {} has a mismatched start time {}", cache.pid, process.start_time()).into());
            }
            if cache.name != process.name() {
                return Err(anyhow::anyhow!("restored process {} has a mismatched name {}", cache.pid, process.name()).into());
            }

            if let Some(path) = process.exe() {
                if cache.exe != path.to_string_lossy() {
                    return Err(anyhow::anyhow!("restored process {} has a mismatched exe {}", cache.pid, path.to_string_lossy()).into());
                }
            } else {
                return Err(anyhow::anyhow!("restored process {} has no exe path", cache.pid).into());
            }
        }

        let process = ChildType::RescuedChild(cache.pid);
        let pid = process.id().ok_or_else(|| anyhow::anyhow!("process failed, couldnt get pid"))?;
        process.cache(cache.uuid, cache.cluster_path.clone(), cache.post.clone()).await?;
        let current_child = Arc::new(RwLock::new(process));
        let manager = Some(tokio::spawn(Self::manager(
            cache.uuid,
            cache.post,
        ...
        send_process(cache.uuid, pid, ProcessPayloadType::Started, "started process").await?;
    ...
    async fn manager(
        ...
        loop {
            ...
            if update >= 60 {
                if let Err(err) = cluster::edit(&cluster_path, |cluster| {
                    ...
                    tracing::warn!("failed to update playtime for cluster {}: {}", &cluster_path, err);
                }
        ...
        tokio::spawn(async move {
            if let Err(err) = cluster::update_playtime(&cluster_path_in).await {
                tracing::warn!("failed to update playtime for cluster {}: {}", &cluster_path_in, err);
            }
        });
        ...
        if !exit_status == 0 {
            send_process(uuid, current_pid, ProcessPayloadType::Finished, "exited process").await?;
        ...
        let post = if let Some(hook) = post {
            ...
            if let Some(c) = cmd.next() {
                ...
                c.args(&cmd.collect::<Vec<&str>>()).current_dir(cluster_path.full_path().await?);
                Some(c)
            } else {
                None
            }
        ...
        if let Some(mut m_c) = post {
            {
                let mut current_child: tokio::sync::RwLockWriteGuard<'_, ChildType> = current_child.write().await;
                let new_child = m_c.spawn().map_err(IOError::from)?;
                current_pid = new_child.id().ok_or_else(|| anyhow::anyhow!("process failed, couldnt get pid"))?;
                *current_child = ChildType::ChildProcess(new_child);
            }

            send_process(uuid, current_pid, ProcessPayloadType::Modified, "running post hook").await?;
