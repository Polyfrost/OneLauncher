...
use std::{collections::HashSet, env, path::{Path, PathBuf}, process::Command};
use futures::prelude::*;
...
pub async fn locate_java() -> Result<Vec<JavaVersion>, JavaError> {
    ...
    for path in common_java_paths {
        let Ok(java_subs) = std::fs::read_dir(path) else { continue; };
...
pub async fn check_java(paths: HashSet<PathBuf>) -> HashSet<JavaVersion> {
    let java_instances = stream::iter(paths.into_iter())
        .map(|p: PathBuf| {
            tokio::task::spawn(async move { check_java_instance(&p).await })
        })
        .buffer_unordered(64)
        .collect::<Vec<_>>()
        .await;
...
pub async fn check_java_instance(path: &Path) -> Option<JavaVersion> {
    ...
    let java = if path.file_name()?.to_str()? != JAVA_BIN { path.join(JAVA_BIN) } else { path };
    if !java.exists() { return None; }
...
pub fn get_java_version(version: &str) -> Result<(u32, u32), JavaError> {
    ...
    let mut minor = if let Some(minor) = split.next() {
        ...
        major = 1;
        major_opt.ok_or_else(|| JavaError::InvalidVersion(version.to_string()))?.parse::<u32>()?
    };
