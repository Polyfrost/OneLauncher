...
use crate::{data::{ManagedPackage, ManagedVersion}, Result};
...
impl Providers {
    ...
    pub async fn list(&self) -> Result<Vec<ManagedPackage>> {
        ...
        }.await?.into_iter().map(|p| p.into()).collect())
    }
    ...
    pub async fn get_version_for_game_version(&self, id: &str, game_version: &str) -> Result<ManagedVersion> {
        let versions = self.get_versions(id).await?;
        Ok(versions.into_iter().find(|v| v.game_versions.contains(&game_version.to_string())).ok_or(anyhow!("no game version found"))?) // TODO: error handling
    }
