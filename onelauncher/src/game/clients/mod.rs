...
use super::{cluster::{Cluster, Manifest}, minecraft::{MinecraftManifest, MinecraftVersion, ModernArgumentsItemExt}};
...
pub trait ClientTrait<'a>: Send + Sync {
	...
	async fn launch(&self, info: LaunchInfo, mut callbacks: LaunchCallbacks) -> crate::Result<i32> {
        ...
        let mut command = process::Command::new(&info.java);
        let command = command
            .current_dir(&info.setup.game_dir)
...
macro_rules! create_game_client {
    ($props:ident { $($props_name:ident: $props_type:ty),* } $client:ident { $($name:ident: $type:ty),* }) => {
        ...
        #[derive(Debug, Clone)]
        pub struct $client<'a> {
            pub cluster: &'a crate::game::cluster::Cluster,
            pub manifest: &'a crate::game::cluster::Manifest,
            $(pub $name: $type),*
        }
    };
}
...
macro_rules! impl_game_client {
	() => {
		fn get_cluster(&self) -> &'a crate::game::cluster::Cluster
		where
			Self: Sized,
		{
			self.cluster
		...
		fn get_manifest(&self) -> &'a crate::game::cluster::Manifest
		where
			Self: Sized,
		{
			self.manifest
