...
pub fn get_java_model(
	absolute_path: &Path,
	info: &JavaInfo,
) -> LauncherResult<java_versions::ActiveModel> {
	...
	.or_else(|_| info.java_version.parse())
...
pub async fn insert_java_many(
	java: Vec<(PathBuf, JavaInfo)>,
) -> LauncherResult<Vec<java_versions::Model>> {
	let db = &State::get().await?.db;
