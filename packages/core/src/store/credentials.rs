...
use crate::constants::MINECRAFT_SCOPES;
...
impl CredentialsStore {
	...
	async fn refresh(
		...
		force: bool,
	) -> LauncherResult<(DeviceKey, DeviceToken, DateTime<Utc>, bool)> {
	...
	#[tracing::instrument(skip(self))]
	pub async fn begin(&mut self) -> LauncherResult<MinecraftLogin> {
	...
	pub async fn finish(
		...
		code: &str,
		flow: MinecraftLogin,
	) -> LauncherResult<MinecraftCredentials> {
	...
	#[tracing::instrument(skip(self))]
	pub async fn get_default(&mut self) -> LauncherResult<Option<MinecraftCredentials>> {
	...
	#[tracing::instrument(skip(self))]
	pub async fn remove(&mut self, id: Uuid) -> LauncherResult<Option<MinecraftCredentials>> {
...
async fn sisu_authenticate(
	...
	let res = send_signed_request::<RedirectUri>(
		...
		json!({
			...
			"RedirectUri": Core::get().msa_redirect_uri,
			"Sandbox": "RETAIL",
			"TokenType": "code",
			"TitleId": "1794566092"
		}),
...
async fn oauth_token(
	...
	query.insert("client_id", core.msa_client_id.as_str());
	query.insert("code", code);
	query.insert("code_verifier", verify);
	query.insert("grant_type", "authorization_code");
	query.insert("redirect_uri", core.msa_redirect_uri.as_str());
