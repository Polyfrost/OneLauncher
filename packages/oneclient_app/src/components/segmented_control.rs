...
impl<T: Copy + PartialEq + 'static> Component for SegmentedControl<T> {
    fn render(&self) -> impl IntoElement {
        ...
        let active_color = |is_selected: bool| {
            if disabled {
        ...
        rect()
            ...
            .children(self.segments.iter().map(move |seg| {
                let value = seg.value;
                let is_selected = value == active;
                let mut selected = selected;
                let label_text = seg.label.clone();
                ...
                rect()
                    ...
                    .maybe_child(seg.icon.map(|icon| {
                        let mut ic = Icon::new(icon);
                        if let Some(size) = icon_size {
                            ic = ic.size(size);
                        }
                        if tint {
                            ic = ic.color(active_color(is_selected));
                        }
                        ic.into_element()
                    }))
                    .maybe_child(label_text.map(|text| {
                        label()
                            .text(text)
                            .font_size(13.)
                            ...
                            .color(active_color(is_selected))
                            .into_element()
                    }))
                    .into_element()
            }))
