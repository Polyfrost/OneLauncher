...
impl Actions {
    ...
    pub fn take_pending_launch(&self) -> Option<String> {
        if self.station.peek().pending_launch.is_none() {
            return None;
        }
    ...
    pub fn create_cluster_shortcut(&self, cluster_id: ClusterId) {
        ...
        spawn_forever(async move {
            ...
            if let Some(desktop) = crate::shortcut::default_dir() {
            ...
            let request = crate::shortcut::ShortcutRequest {
                cluster_name: cluster.name.clone(),
                folder_name: cluster.folder_name.clone(),
                dir: handle.path().to_path_buf(),
            };

            let written =
                tokio::task::spawn_blocking(move || crate::shortcut::create(&request)).await;
            ...
            match written {
                Ok(Ok(path)) => {
                    ...
                    actions
                        ...
                        .body(format!("Saved as {name} in {}", handle.path().display()))
                        .icon(IconType::LinkExternal01)
                        .send();
                }
