...
pub(super) fn ensure_allowed(path: &Path) -> ClusterResult<PathBuf> {
    polyio::ensure_under(path, [paths::logs_dir()?, paths::clusters_dir()?])?
...
pub fn cluster_output_log(cluster: &Cluster) -> ClusterResult<PathBuf> {
...
pub(super) async fn read_file_string(path: &Path) -> ClusterResult<String> {
    let is_gz = path
        .extension()
        .and_then(|e| e.to_str())
...
pub async fn read_log_at(path: &Path, opts: &ReadOptions) -> ClusterResult<Vec<LogLine>> {
...
pub async fn delete_log_at(path: &Path) -> ClusterResult<()> {
    let path = ensure_allowed(path)?;
