...
async fn tail(
    ...
    loop {
        ...
        if !line.ends_with('\n') {
            ...
            reader
                .seek(std::io::SeekFrom::Start(offset - read as u64))
                .await?;
            offset -= read as u64;
            tokio::time::sleep(POLL_INTERVAL).await;
            continue;
...
mod tests {
    ...
    impl Harness {
        ...
        fn drain(&mut self) -> Vec<String> {
            let mut out = Vec::new();
            while let Ok(oneclient_events::Event::Game(oneclient_events::GameEvent::Log {
                line,
                ..
            })) = self.rx.try_recv()
            {
    ...
    async fn stops_even_when_the_last_line_has_no_newline() {
        ...
        polyio::write(&path, "[12:00:00] cut off mid-writ")
            .await
            .unwrap();
        ...
        assert!(
            stopped.is_ok(),
            "tail.stop() hung on an unterminated final line"
        );
