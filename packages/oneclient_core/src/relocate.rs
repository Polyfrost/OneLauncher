...
async fn drop_materialized_content(state: &LauncherState) {
    ...
    match state.clusters.list().await {
        ...
        Err(err) => {
            tracing::warn!(%err, "could not list clusters; only clearing the shared folder")
        }
    }
...
async fn write_everything(
    ...
    if rewritten > 0 {
        tracing::info!(
            rows = rewritten,
            "repointed Java runtimes at the new folder"
        );
    }
...
async fn commit(state: &LauncherState, from: &Path, to: &Path) -> Result<(), String> {
    ...
    save_settings(&snapshot)
        .await
        .map_err(|err| format!("Everything was copied but your settings could not be saved: {err}"))
}
...
mod tests {
    ...
    async fn the_walk_skips_what_it_is_told_to_and_counts_the_rest() {
        ...
        let found = collect(dir, &[OsString::from("user_data.db")])
            .await
            .unwrap();
        ...
        assert_eq!(
            found.dirs.len(),
            1,
            "the folder itself still has to be made"
        );
