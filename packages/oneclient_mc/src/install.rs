...
const NATIVE_SUFFIX: &str = if cfg!(windows) {
    ".dll"
...
pub async fn check_natives(
    ...
    for lib in natives_for_host(version, java_arch, minecraft_updated)? {
        let verdict = if native_download(lib, java_arch).is_some() {
            let unpacked = std::fs::read_dir(&natives_dir).is_ok_and(|entries| {
                entries
                    .filter_map(Result::ok)
                    .any(|entry| entry.file_name().to_string_lossy().ends_with(NATIVE_SUFFIX))
            });
...
pub async fn verify_game_files(
    ...
    let natives = check_natives(version, java_arch, minecraft_updated).await?;
...
pub fn natives_missing(
    ...
    for lib in natives_for_host(version_info, java_arch, minecraft_updated)? {
