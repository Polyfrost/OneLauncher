...
impl NetConfig {
	...
	///
	/// Non-public hosts (localhost, private IPs) are normally refused for
	/// SSRF safety, but a local dev backend can only be reached when it was
	/// deliberately configured here, so its host is allowed with the same
