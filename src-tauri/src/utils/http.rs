...
pub async fn download_file(url: &str, path: &Path) -> PolyResult<()> {
    let response = create_client()?.get(url).send().await.map_err(|err| PolyError::HTTPError(err))?;
    let mut file = File::create(path).map_err(|err| PolyError::IOError(err))?;
    let mut content = Cursor::new(response.bytes().await.map_err(|err| PolyError::HTTPError(err))?);
